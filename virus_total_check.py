...
VIRUSTOTALAPIKEY = "InsertVirusTotalAPIKeyHERE"
...
file_path = sys.argv[1].strip() if len(sys.argv) > 1 else sys.exit()
...
def check_file(file_path, text_widget):
    ...
    with open(file_path, 'rb') as file:
        ...
        if response.status_code == 200:
            ...
            while analysis_response.json()['data']['attributes']['status'] != 'completed':
                sleep(1)
...
def create_window():
    ...
    window.iconbitmap(join(base_path, 'icon.ico'))
